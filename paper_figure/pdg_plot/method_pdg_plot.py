...
# Figure layout
# ============================================================================
FIG_W, FIG_H = 7.0, 2.05

fig = plt.figure(figsize=(FIG_W, FIG_H))
gs = fig.add_gridspec(
    1, 3,
    width_ratios=[1.45, 1.18, 1.05],
    wspace=0.025,
    left=0.006, right=0.994, bottom=0.020, top=0.980,
)
...
# Code box (left ~46% of panel)
code_x0, code_x1 = 0.020, 0.460
code_y0, code_y1 = 0.180, 0.905
...
for i, (line, kind) in enumerate(CODE_LINES):
    ...
    axA.text(code_x0 + 0.008, y, line, fontsize=4.8,
...
nodes = {
    "add":      dict(xy=(0.595, 0.78), fill=TOPFN_FILL, edge=TOPFN_EDGE),
    "is_int":   dict(xy=(0.595, 0.62), fill=TOPFN_FILL, edge=TOPFN_EDGE),
    "__init__": dict(xy=(0.830, 0.78), fill=CLSFN_FILL, edge=CLSFN_EDGE),
    "push":     dict(xy=(0.830, 0.62), fill=CLSFN_FILL, edge=CLSFN_EDGE),
    "total":    dict(xy=(0.830, 0.46), fill=CLSFN_FILL, edge=CLSFN_EDGE),
    "mean":     dict(xy=(0.830, 0.30), fill=CLSFN_FILL, edge=CLSFN_EDGE),
}
...
axA.text(cls_x0, cls_y1 + 0.022,
         "class Calculator",
         fontsize=5.6, family="monospace", color="#345c44",
         style="italic", ha="left", va="bottom", zorder=10)
...
def call_edge(ax, src, dst, rad=0.0, color="#404040", shrinkA=11, shrinkB=11):
    p, q = nodes[src]["xy"], nodes[dst]["xy"]
    arr = FancyArrowPatch(
        p, q, connectionstyle=f"arc3,rad={rad}",
        arrowstyle="-|>", mutation_scale=8.0,
        linewidth=0.75, color=color,
        shrinkA=shrinkA, shrinkB=shrinkB, zorder=7,
    )
    ax.add_patch(arr)
...
call_edge(axA, "mean",  "total", rad=0.0, shrinkA=4, shrinkB=4)
...
# E_call arrow
axA.annotate(
    "", xy=(0.075, leg_y), xytext=(0.025, leg_y),
    arrowprops=dict(arrowstyle="-|>", linewidth=0.75,
                    color="#404040", mutation_scale=7),
)
axA.text(0.090, leg_y, r"$\mathcal{E}_{\mathrm{call}}$",
         fontsize=6.4, va="center", ha="left", color=TEXT_DK)

# E_sib dashed line
axA.add_line(Line2D(
    [0.205, 0.260], [leg_y, leg_y],
    linewidth=0.7, linestyle=(0, (2.5, 1.8)), color="#5e7a68",
))
axA.text(0.275, leg_y, r"$\mathcal{E}_{\mathrm{sib}}$",
         fontsize=6.4, va="center", ha="left", color=TEXT_DK)

# top-level swatch
axA.add_patch(Rectangle((0.395, leg_y - 0.018), 0.026, 0.034,
                        facecolor=TOPFN_FILL, edgecolor=TOPFN_EDGE,
                        linewidth=0.4))
axA.text(0.430, leg_y, "top-level",
         fontsize=6.0, va="center", ha="left", color=TEXT_DK)

# in-class swatch
axA.add_patch(Rectangle((0.610, leg_y - 0.018), 0.026, 0.034,
                        facecolor=CLSFN_FILL, edgecolor=CLSFN_EDGE,
                        linewidth=0.4))
axA.text(0.645, leg_y, "in-class",
         fontsize=6.0, va="center", ha="left", color=TEXT_DK)
...
axB.add_patch(FancyBboxPatch(
    (0.060, fim_box_y0), 0.880, fim_box_y1 - fim_box_y0,
...
axB.text(0.500, 0.140,
         ...
         r"\geq\tau{=}0.08\Rightarrow\mathrm{selected}$",
         fontsize=6.2, ha="center", va="center", color=SEL_FG,
