...
# ============================================================================
# Figure layout — narrower / taller so text survives \linewidth scaling
# ============================================================================
FIG_W, FIG_H = 8.0, 7.5
...
CODE_LINES = [
    ...
    ("        return self.total() / len(self.history)", "code"),
]

# Code box
code_x0, code_x1 = 0.020, 0.450
code_y0, code_y1 = 0.080, 0.910
...
for i, (line, kind) in enumerate(CODE_LINES):
    ...
    axA.text(code_x0 + 0.010, y, line, fontsize=8.0,
             family="monospace", color=col, style=style,
             ha="left", va="center", zorder=2)

# Arrow "AST parse →" between code and PDG
axA.annotate(
    "", xy=(0.523, 0.50), xytext=(0.470, 0.50),
    arrowprops=dict(arrowstyle="->", linewidth=1.0, color="#666"),
)
axA.text(0.4965, 0.555, "AST\nparse", fontsize=7.5,
         color=TEXT_MD, ha="center", va="bottom", family="serif",
         linespacing=1.0)

# ---- PDG nodes (right ~50% of Panel A) ----
NODE_W, NODE_H = 0.140, 0.110

nodes = {
    "add":      dict(xy=(0.625, 0.78), fill=TOPFN_FILL, edge=TOPFN_EDGE),
    "is_int":   dict(xy=(0.625, 0.62), fill=TOPFN_FILL, edge=TOPFN_EDGE),
...
# class container box (drawn behind nodes)
...
axA.text(cls_x0 + 0.012, cls_y1 - 0.014, "class Calculator",
...
# Sibling edges down the right side of class methods
...
# Legend at the bottom of Panel A
leg_y = 0.040
axA.annotate(
    "", xy=(0.555, leg_y + 0.005), xytext=(0.495, leg_y + 0.005),
    arrowprops=dict(arrowstyle="-|>", linewidth=0.85,
                    color="#404040", mutation_scale=8.5),
)
axA.text(0.575, leg_y + 0.005, r"$\mathcal{E}_{\mathrm{call}}$",
         fontsize=9.0, va="center", ha="left", color=TEXT_DK)

axA.add_line(Line2D(
    [0.660, 0.730], [leg_y + 0.005, leg_y + 0.005],
    linewidth=0.9, linestyle=(0, (2.5, 1.8)), color="#5e7a68",
))
axA.text(0.745, leg_y + 0.005, r"$\mathcal{E}_{\mathrm{sib}}$",
         fontsize=9.0, va="center", ha="left", color=TEXT_DK)

axA.add_patch(Rectangle((0.825, leg_y - 0.012), 0.030, 0.034,
                        facecolor=CLSFN_FILL, edgecolor=CLSFN_EDGE,
                        linewidth=0.5))
axA.text(0.862, leg_y + 0.005, "method",
         fontsize=8.4, va="center", ha="left", color=TEXT_DK)

axA.add_patch(Rectangle((0.925, leg_y - 0.012), 0.030, 0.034,
                        facecolor=TOPFN_FILL, edgecolor=TOPFN_EDGE,
                        linewidth=0.5))
axA.text(0.962, leg_y + 0.005, "top-level",
         fontsize=8.4, va="center", ha="left", color=TEXT_DK)
...
bar_x0, bar_x1 = 0.080, 0.815
bar_w = bar_x1 - bar_x0
bar_h = 0.058


def stacked_bar(ax, y, values, colors, total, total_label, total_color):
    """Draw stacked bar + segment value labels inside; total at the right."""
    x = bar_x0
    for v, c in zip(values, colors):
        w = (v / total) * bar_w
        ax.add_patch(Rectangle((x, y), w, bar_h, facecolor=c,
                               edgecolor="none", zorder=3))
        # luminance for text colour
    ...
    ax.text(bar_x1 + 0.018, y + bar_h / 2, total_label,
...
# ---- Hhat block --------------------------------------------------------
H_TOTAL = 0.45
H_VALS  = [0.12, 0.18, 0.15]   # contribution of each component
H_INPUTS = ["LoC=10", "CC=4", "D=2"]
H_NAMES  = ["LoC", "CC", "D"]

axB.text(0.040, 0.880,
         r"$\hat{H}\;=\;w_{\ell}\,\phi(\mathrm{LoC})"
         r"\,+\,w_{c}\,\phi(\mathrm{CC})"
         r"\,+\,w_{d}\,\phi(\mathrm{D})$",
         fontsize=9.0, ha="left", va="top", color=TEXT_DK)

# inputs row above the bar — each input sits over its segment
H_BAR_Y = 0.745
input_y = H_BAR_Y + bar_h + 0.025
seg_left = bar_x0
for v, name, inp in zip(H_VALS, H_NAMES, H_INPUTS):
    seg_w = (v / H_TOTAL) * bar_w
    cx = seg_left + seg_w / 2
    axB.text(cx, input_y, inp, fontsize=8.0,
             ha="center", va="center", color=TEXT_DK, family="serif")
    seg_left += seg_w

# arrow row from inputs into the bar
arrow_y0 = input_y - 0.024
arrow_y1 = H_BAR_Y + bar_h + 0.005
seg_left = bar_x0
for v in H_VALS:
    seg_w = (v / H_TOTAL) * bar_w
    cx = seg_left + seg_w / 2
    axB.annotate("", xy=(cx, arrow_y1), xytext=(cx, arrow_y0),
                 arrowprops=dict(arrowstyle="->", linewidth=0.55,
                                 color="#888", mutation_scale=6))
    seg_left += seg_w

stacked_bar(axB, H_BAR_Y, H_VALS, H_COLS, H_TOTAL,
            r"$=\hat{H}{=}0.45$", H_COLS[0])

# segment legend below H bar
seg_y = H_BAR_Y - 0.040
seg_left = bar_x0
for v, name, c in zip(H_VALS, H_NAMES, H_COLS):
    seg_w = (v / H_TOTAL) * bar_w
    cx = seg_left + seg_w / 2
    axB.text(cx, seg_y, name, fontsize=7.6,
             ha="center", va="center", color=c, family="serif",
             weight="bold")
    seg_left += seg_w

# ---- Ihat block --------------------------------------------------------
axB.text(0.040, 0.520,
         r"$\hat{I}\;=\;\alpha C_{\mathrm{caller}}+\beta C_{\mathrm{callee}}"
         r"+\gamma C_{\mathrm{sig}}+\delta C_{\mathrm{doc}}"
         r"+\varepsilon C_{\mathrm{class}}$",
         fontsize=8.6, ha="left", va="top", color=TEXT_DK)

I_TOTAL = 0.65
I_VALS  = [0.08, 0.14, 0.14, 0.05, 0.24]
I_NAMES = ["caller", "callee", "sig", "doc", "class"]
I_INPUTS = ["1", "2", "type+name", r"$\checkmark$", "3 sibs"]

I_BAR_Y = 0.380
input_y = I_BAR_Y + bar_h + 0.025
seg_left = bar_x0
for v, inp in zip(I_VALS, I_INPUTS):
    seg_w = (v / I_TOTAL) * bar_w
    cx = seg_left + seg_w / 2
    axB.text(cx, input_y, inp, fontsize=7.4,
             ha="center", va="center", color=TEXT_DK, family="serif")
    seg_left += seg_w

# arrows
arrow_y0 = input_y - 0.024
arrow_y1 = I_BAR_Y + bar_h + 0.005
seg_left = bar_x0
for v in I_VALS:
    seg_w = (v / I_TOTAL) * bar_w
    cx = seg_left + seg_w / 2
...
stacked_bar(axB, I_BAR_Y, I_VALS, I_COLS, I_TOTAL,
            r"$=\hat{I}{=}0.65$", I_COLS[-1])

# segment names below
seg_y = I_BAR_Y - 0.040
seg_left = bar_x0
for v, name, c in zip(I_VALS, I_NAMES, I_COLS):
    seg_w = (v / I_TOTAL) * bar_w
    cx = seg_left + seg_w / 2
    axB.text(cx, seg_y, name, fontsize=7.0,
             ha="center", va="center", color=c, family="serif",
             weight="bold")
    seg_left += seg_w

# ---- combined FIM at the bottom ----------------------------------------
fim_box_y0, fim_box_y1 = 0.040, 0.215
axB.add_patch(FancyBboxPatch(
    (0.040, fim_box_y0), 0.920, fim_box_y1 - fim_box_y0,
    boxstyle="round,pad=0.005,rounding_size=0.018",
    linewidth=0.45, edgecolor=SEL_FG, facecolor=SEL_BG, zorder=2,
))
axB.text(0.060, 0.170,
         r"$\mathrm{FIM}=\dfrac{\hat{H}\,\hat{I}}{\hat{H}+\hat{I}+\epsilon}"
         r"=\dfrac{0.45\times 0.65}{0.45+0.65}\;\approx\;0.27$",
         fontsize=9.0, ha="left", va="top", color=TEXT_DK)
axB.text(0.060, 0.075,
         r"$\geq\tau\;\Rightarrow\;\mathtt{Calculator.total}$"
         r" is selected as a mask target.",
         fontsize=8.5, ha="left", va="center", color=SEL_FG,
...
# Inset axis with real ticks
inset = axC.inset_axes([0.155, 0.130, 0.795, 0.760],
...
label_iso(0.20, 0.36, color=THR_EDGE, weight="bold",
          text=fr"$\tau={tau:.2f}$")
...
# Annotation: filtered cluster
inset.annotate(
    "hard-filtered\n(LoC<10)",
    xy=(0.10, 0.35), xytext=(0.32, 0.55),
...
# Annotation: selected region
inset.annotate(
    "selected\nregion",
    xy=(0.85, 0.32), xytext=(0.70, 0.13),
...
# Legend (top-left where there are no points)
leg_handles = [
