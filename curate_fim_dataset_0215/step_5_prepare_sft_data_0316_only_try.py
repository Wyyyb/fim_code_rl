...
Updated: adds per-sample metadata for downstream ablation experiments.
"""
...
def compute_statistics(records: list[dict], sft_samples: list[dict]) -> dict:
    ...
    stats = {
        ...
        'quality_rating_stats': defaultdict(int),
    }
    ...
    for record in records:
        ...
        # Repo / category / quality_rating breakdown
        stats['repo_stats'][record.get('description', 'unknown')] += 1
        stats['category_stats'][record.get('category', 'unknown')] += 1
        stats['quality_rating_stats'][record.get('quality_rating', 'unknown')] += 1
...
def print_statistics(filter_stats: dict, data_stats: dict):
    ...
    qr_stats = data_stats.get('quality_rating_stats', {})
    if qr_stats:
        print(f"\n⭐ Quality Rating Breakdown:")
        for qr, cnt in sorted(qr_stats.items(), key=lambda x: -x[1]):
            print(f"      {qr:20s}  {cnt:,}")
...
Examples:
  ...
  python prepare_sft_data_0219.py -d /path/to/shards -o output.jsonl \\
    ...
    parser.add_argument(
        "--checkpoint-dir", "-d",
        default="/data/yubo/datasets/fim_mt_single_function_data_v2_only_try/",
        help="Directory containing shard checkpoint files"
    )
    parser.add_argument(
        "--output", "-o",
        default="/data/yubo/datasets/hf_fim_mt_single_function_data_v2_0228/fim_mt_single_function_data_v2_0228_filtered_0316.jsonl",
    ...
    parser.add_argument(
        "--save-stats",
        type=str,
        default="/data/yubo/datasets/hf_fim_mt_single_function_data_v2_0228/stats_filtered_0316.json",
    ...
    if args.save_stats:
        stats_output = {
            ...
            'data_stats': {
                ...
                'quality_rating_stats': dict(data_stats.get('quality_rating_stats', {})),
            }
        }
