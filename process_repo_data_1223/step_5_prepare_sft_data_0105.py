#!/usr/bin/env python3
"""
SFT Training Data Preparation Script

This script processes checkpoint files from step 4 and generates SFT training data.
It filters low-quality samples and formats them for instruction fine-tuning.
"""
...
def load_checkpoint_files(checkpoint_dir: str, checkpoint_pattern: str = "*checkpoint*.json") -> list[dict]:
    """
    Load all checkpoint files from a directory.
    ...
    Returns:
        List of all records from all checkpoint files
    """
    ...
    checkpoint_files = list(checkpoint_path.glob(checkpoint_pattern))

    if not checkpoint_files:
        print(f"⚠️ No checkpoint files found matching pattern '{checkpoint_pattern}' in {checkpoint_dir}")
        return all_records
    ...
    for cp_file in sorted(checkpoint_files):
        try:
            ...
            # Checkpoint files have 'results' key containing the actual records
            results = data.get('results', [])
            all_records.extend(results)
            loaded_files += 1
            print(f"   ✅ Loaded {len(results)} records from {cp_file.name}")

        except Exception as e:
            print(f"   ❌ Failed to load {cp_file.name}: {e}")
...
def filter_low_quality_samples(
        records: list[dict],
        min_individual_score: int = 2,
        min_overall_score: int = 3
    ...
    Args:
        ...
        min_individual_score: Minimum score for individual dimensions (exclusive)
        min_overall_score: Minimum overall score (exclusive)
    ...
    for record in records:
        ...
        # Check overall score (must be > min_overall_score, i.e., >= 3)
        overall_score = critique_response.get('overall_score', 0)
        if overall_score <= min_overall_score - 1:  # <= 2 means filtered
            filter_stats['filtered_low_overall_score'] += 1
            continue

        # Check individual scores (must be >= min_individual_score, i.e., >= 2)
        scores = critique_response.get('scores', {})
        low_score_found = False
        for key in individual_score_keys:
            score = scores.get(key, 0)
            if score < min_individual_score:  # < 2 means filtered
...
def construct_sft_sample(record: dict) -> Optional[dict]:
    ...
    Args:
        record: A filtered record containing FIM response

    Returns:
        SFT sample in messages format, or None if construction fails
    """
    try:
        masked_code = record.get('masked_code', '')
        ...
        # Construct user message (same as FIM completion prompt)
        user_content = FIM_COMPLETION_PROMPT.format(
            masked_code=masked_code,
            function_name=function_name
        )

        # Construct assistant message (the FIM response)
        # Use the raw response which includes reasoning and implementation
...
def compute_statistics(
    ...
    Args:
        ...
        sft_samples: Constructed SFT samples
...
def print_statistics(filter_stats: dict, data_stats: dict):
    ...
    print(f"   Filtered (low individual):     {filter_stats['filtered_low_individual_score']:,}")
    print(f"   Filtered (low overall):        {filter_stats['filtered_low_overall_score']:,}")
...
def save_sft_data(sft_samples: list[dict], output_path: str):
    ...
    Args:
        sft_samples: List of SFT samples
...
def main():
    parser = argparse.ArgumentParser(
        description="Prepare SFT training data from step 4 checkpoints",
...
Examples:
  ...
  python prepare_sft_data.py -d ./checkpoints -o sft_train.jsonl \\
    ...
    parser.add_argument(
        ...
        help="Glob pattern for checkpoint files (default: *checkpoint*.json)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="Minimum overall score (exclusive, default: 3, meaning scores <= 2 are filtered)"
    )
    parser.add_argument(
        "--save-stats",
        type=str,
    ...
    print("\n📂 Step 1: Loading checkpoint files...")
    all_records = load_checkpoint_files(args.checkpoint_dir, args.checkpoint_pattern)
    ...
    print(f"   - Individual scores must be >= {args.min_individual_score}")
    print(f"   - Overall score must be > {args.min_overall_score - 1} (i.e., >= {args.min_overall_score})")
