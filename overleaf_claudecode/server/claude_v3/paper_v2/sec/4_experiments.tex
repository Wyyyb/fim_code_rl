...
selected FIM corpus using the standard FIM loss on the middle span
...
All numerical results are means over three independent evaluation seeds
on the final checkpoint of each pipeline; the ``\%'' symbol is omitted
inside table cells. Standard-deviation bands shown in red
(\textcolor{red}{$\pm$?}) are placeholders awaiting final replication
runs. ``Baseline'' refers to the base model followed by an agentic
post-training pipeline; ``ours'' refers to the same base followed by FIM
mid-training and then by the identical post-training pipeline. We do not
evaluate mid-training-only checkpoints, because FIM data without
subsequent instruction-style post-training degrades instruction-following
and makes any comparison against instruction-tuned baselines unfair.
Every gain we attribute to mid-training is therefore one that
...
r2egym}; SWE-Smith uses SWE-agent~\citep{sweagent}; the Qwen3-8B runs use
OpenHands directly because SWE-Lego post-training data exceeds the 32K
context of the R2E-Gym/SWE-Smith scaffolds).
...
\midrule
\multicolumn{4}{l}{\textit{Qwen2.5-Coder-32B-Instruct}} \\
\quad --- (no agentic training)                       & 7.0\,($\pm$1.3)            & 3.0\,($\pm$1.4)            & 5.00 \\
\quad + R2E-Gym (reproduced)                          & 31.80\,($\pm$1.6)          & 24.70\,($\pm$1.4)          & 28.25 \\
\quad + R2E-Gym (officially reported)                 & 34.40\,($\pm$1.2)          & 23.77\,($\pm$0.8)          & 29.09 \\
\quad \textbf{+ FIM-Midtrain + R2E-Gym}               & \textbf{35.10}\,($\pm$1.7) & \textbf{26.80}\,($\pm$1.3) & \textbf{30.95} \\
\rowcolor{blue!8}
\quad\quad $\Delta$ (ours vs.\ reproduced)            & $+3.30$                    & $+2.10$                    & $+2.70$ \\
\midrule
...
\textbf{Consistent gains across model scales.}
Holding the post-training pipeline fixed at R2E-Gym, FIM mid-training
improves SWE-Bench-Verified by $+2.80$ at 7B, $+3.00$ at 14B, and
$+3.30$ at 32B; the absolute gain stays in a narrow band as the base
model grows by $4{\times}$ in parameters. SWE-Bench-Lite shows the same
direction at every scale ($+3.67/+4.00/+2.10$). We read this as evidence
that the structural prior is not absorbed by larger pretrained models in
the practical deployment range, not as a claim of monotone scaling
beyond 32B.
...
\textbf{Transfer across base-model families.}
Switching to Qwen3-8B and SWE-Lego, mid-training improves Verified by
$+3.20$ and Lite by $+5.40$, comparable to the Qwen2.5-Coder gains. This
comparison varies the post-training pipeline simultaneously with the
base model (SWE-Lego is required because Qwen3-8B's $40{,}960$-token
context exceeds that of the R2E-Gym/SWE-Smith scaffolds), so the
cross-base-model result should be read as ``not specific to the
Qwen2.5-Coder family under one alternative agentic pipeline.'' Full
Qwen2.5-Coder scaling curves are in Appendix~\ref{app:scaling}.
...
We run two controlled ablations on Qwen2.5-Coder-7B with R2E-Gym
...
Bold marks the best configuration per block; the $85\%/10\%/5\%$ mixture
...
\textbf{85\% single + 10\% pair + 5\% triple} & \textbf{17.40} & \textbf{14.80} & \textbf{16.10} \\
\bottomrule
\end{tabular}
\end{table}

\textbf{FIM structure contributes independently of CoT distillation.}
Block~(A) addresses a natural concern that improvements stem
mainly from distilling reasoning. Removing the rationale entirely
(\emph{no CoT}) still improves the average by $+0.13$ over the baseline,
with $+0.60$ on Verified, indicating the function-aware FIM objective
contributes some signal on its own. Replacing Gemini-3 with rationales
...
the gain is reproducible without a frontier teacher.
