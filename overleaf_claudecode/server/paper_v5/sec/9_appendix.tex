...
and a cosine learning-rate schedule; LlamaFactory runs use DeepSpeed
ZeRO-3, FlashAttention-2, and the Liger kernel, and the torchtune run
uses fused AdamW, activation checkpointing, and \texttt{torch.compile}.
For R2E-Gym and SWE-Smith we follow the official release scripts; for
SWE-Lego we use $2$ epochs instead of the official $4$ to prevent
overfitting (all other hyperparameters unchanged). Table~\ref{tab:hp}
lists the exact settings for the four training stages; effective
(global) batch sizes assume $8$ GPUs.

\begin{table}[h]
\centering
\footnotesize
\caption{Hyperparameters for the four training stages: (a) FIM
mid-training applied uniformly to Qwen2.5-Coder-7B-Instruct,
Qwen2.5-Coder-14B-Instruct, and Qwen3-8B; (b)--(d) the three agentic
...
\textbf{Hyperparameter}
  & \textbf{(a) FIM Mid-Train}
  & \textbf{(b) R2E-Gym}
  & \textbf{(c) SWE-Smith}
  & \textbf{(d) SWE-Lego} \\
\midrule
Framework             & LlamaFactory & LlamaFactory & torchtune & LlamaFactory \\
Base model            & Qwen2.5-Coder \{7B,14B\}, Qwen3-8B
                      & Qwen2.5-Coder-7B-Inst.
                      & Qwen2.5-Coder-7B-Inst.
                      & Qwen3-8B (FIM-midtrained) \\
Optimizer             & AdamW & AdamW & AdamW (fused) & AdamW \\
Learning rate         & $1.0\!\times\!10^{-5}$
                      & $1.0\!\times\!10^{-5}$
                      & $1.0\!\times\!10^{-4}$
                      & $1.0\!\times\!10^{-4}$ \\
LR schedule           & Cosine & Cosine & Cosine & Cosine \\
Warmup                & ratio $0.1$ & ratio $0.05$ & $5$ steps & ratio $0.1$ \\
Weight decay          & $0.05$ & $0.0$ & $0.01$ & $0.01$ \\
Epochs                & $1$ & $2$ & $3$ & $2$\textsuperscript{$\dagger$} \\
Per-device batch size & $1$ & $1$ & $1$ & $1$ \\
Gradient accumulation & $16$ & $1$ & $4$ & $8$ \\
Effective batch size  & $128$ & $8$ & $32$ & $64$ \\
Sequence length       & $32{,}768$ & $20{,}480$ & $32{,}768$ & $40{,}960$ \\
Precision             & bf16 & bf16 & bf16 & bf16 \\
RoPE scaling          & YaRN & --- & --- & YaRN \\
System                & ZeRO-3, FA-2, Liger
                      & ZeRO-3 (offload), FA-2, Liger
                      & FSDP, act.\ ckpt., \texttt{compile}
                      & ZeRO-3, FA-2, Liger \\
\bottomrule
\end{tabular}
\\[0.3em]
