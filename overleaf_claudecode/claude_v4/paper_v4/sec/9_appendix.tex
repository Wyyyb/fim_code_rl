...
licenses (MIT, Apache-2.0, BSD-2/3, ISC, Boost, MIT-0, Unlicense, WTFPL),
followed by copyleft (GPL-2/3, AGPL-3, LGPL-2.1/3, MPL-2.0, BSD-3 Clear)
and Creative Commons licenses; per-license shares are
\textcolor{red}{TBD} pending final tabulation. We will release the
per-repository license alongside the repository list.

\begin{table}[h]
\centering
\caption{License inventory of the $968$-repository corpus. Counts are
...
\midrule
MIT License                                     & \textcolor{red}{TBD} & GNU AGPL v3.0                                  & \textcolor{red}{TBD} \\
Apache License 2.0                              & \textcolor{red}{TBD} & CC BY-SA 4.0                                   & \textcolor{red}{TBD} \\
BSD 3-Clause                                    & \textcolor{red}{TBD} & CC BY 4.0                                      & \textcolor{red}{TBD} \\
BSD 2-Clause                                    & \textcolor{red}{TBD} & CC0 1.0                                        & \textcolor{red}{TBD} \\
GNU GPL v3.0                                    & \textcolor{red}{TBD} & MPL 2.0                                        & \textcolor{red}{TBD} \\
GNU GPL v2.0                                    & \textcolor{red}{TBD} & GNU LGPL v2.1                                  & \textcolor{red}{TBD} \\
ISC License                                     & \textcolor{red}{TBD} & GNU LGPL v3.0                                  & \textcolor{red}{TBD} \\
MIT-0                                           & \textcolor{red}{TBD} & CC BY-NC 4.0                                   & \textcolor{red}{TBD} \\
The Unlicense                                   & \textcolor{red}{TBD} & BSD 3-Clause Clear                             & \textcolor{red}{TBD} \\
Boost Software License 1.0                      & \textcolor{red}{TBD} & Other / unidentifiable                         & \textcolor{red}{TBD} \\
\midrule
\multicolumn{4}{l}{\textbf{Total: 968}} \\
\bottomrule
\end{tabular}
\end{table}
...
and a cosine learning-rate schedule. The \textsc{LlamaFactory} runs use
DeepSpeed ZeRO-3 sharding, FlashAttention-2, and the Liger kernel; the
\textsc{torchtune} run uses fused AdamW, activation checkpointing, and
\texttt{torch.compile}. For R2E-Gym and SWE-Smith we follow the
official release scripts; for SWE-Lego we follow the official recipe
except that we train for $2$ epochs instead of $4$ to prevent
overfitting (all other hyperparameters are unchanged).
Tables~\ref{tab:hp-midtrain} and~\ref{tab:hp-posttrain} list the exact
settings; effective (global) batch sizes assume $8$ GPUs.

\begin{table}[h]
\centering
\footnotesize
\caption{FIM mid-training hyperparameters (\textsc{LlamaFactory}). The
same recipe is applied to all three base models
(Qwen2.5-Coder-7B-Instruct, Qwen2.5-Coder-14B-Instruct, and
Qwen3-8B).}
...
Effective batch size & $128$ (8\,GPUs $\times 1 \times 16$) \\
Sequence length      & $32{,}768$ \\
Precision            & bf16 \\
RoPE scaling         & YaRN \\
System               & DeepSpeed ZeRO-3, FlashAttention-2, Liger kernel \\
\bottomrule
\end{tabular}
\end{table}
...
follows the official recipe except for the epoch count ($2$ instead of
...
\midrule
Framework             & LlamaFactory & torchtune & LlamaFactory \\
Base model            & Qwen2.5-Coder-7B-Inst. & Qwen2.5-Coder-7B-Inst. & Qwen3-8B (FIM mid-trained) \\
Optimizer             & AdamW & AdamW (fused) & AdamW \\
Learning rate         & $1.0\!\times\!10^{-5}$ & $1.0\!\times\!10^{-4}$ & $1.0\!\times\!10^{-4}$ \\
LR schedule           & Cosine & Cosine & Cosine \\
Warmup                & ratio $0.05$ & $5$ steps & ratio $0.1$ \\
Weight decay          & $0.0$ (default) & $0.01$ & $0.01$ \\
Epochs                & $2$ & $3$ & $2$\textsuperscript{$\dagger$} \\
Per-device batch size & $1$ & $1$ & $1$ \\
Gradient accumulation & $1$ & $4$ & $8$ \\
Effective batch size  & $8$ & $32$ & $64$ \\
Sequence length       & $20{,}480$ & $32{,}768$ & $131{,}072$ \\
Precision             & bf16 & bf16 & bf16 \\
System                & ZeRO-3 (offload), FA-2, Liger & FSDP, act.\ ckpt., \texttt{compile} & ZeRO-3, FA-2, Liger \\
\bottomrule
\end{tabular}
\\[0.4em]
{\raggedright \footnotesize \textsuperscript{$\dagger$}We deviate from
the official SWE-Lego recipe ($4$ epochs) and use $2$ epochs to prevent
overfitting; all other hyperparameters follow the official scripts.\par}
\end{table}
