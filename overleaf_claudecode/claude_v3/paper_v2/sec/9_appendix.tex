...
%   C.  Training Hyperparameters (stub)
%   D.  Full Qwen2.5-Coder Scaling Curves (stub)
%   E.  Extended Behavioral Analysis (Verified)
%   F.  Behavioral Analysis on SWE-Bench-Lite
% =============================================================================
...
statistics, and the license inventory of the $990$ released repositories.
...
Red entries are placeholders awaiting final replication runs.}
\label{tab:dataset}
\small
...
Table~\ref{tab:licenses} summarizes the license distribution across the
$990$ released repositories. The corpus is overwhelmingly permissive:
$\sim\!79\%$ of repositories carry a permissive license (MIT,
Apache-2.0, BSD-2/3, ISC, Boost, MIT-0, Unlicense, WTFPL); $\sim\!5\%$
carry a copyleft license (GPL-2/3, AGPL-3, LGPL-2.1/3, MPL-2.0, BSD-3
Clear); and $\sim\!2\%$ carry Creative Commons licenses, with the
remainder either lacking a declared license or unidentifiable by the
crawler. We will release the per-repository license alongside the
repository list.

\begin{table}[h]
\centering
\caption{License inventory of the $990$-repository corpus. Counts are
exact at release time; a small number of repositories with no declared
or unidentifiable license are reported separately.}
\label{tab:licenses}
\small
\setlength{\tabcolsep}{6pt}
\begin{tabular}{lr|lr}
...
\midrule
MIT License                                     & $350$ & GNU AGPL v3.0                                  & $17$ \\
Apache License 2.0                              & $370$ & CC BY-SA 4.0                                   & $17$ \\
BSD 3-Clause                                    & $101$ & CC BY 4.0                                      & $7$  \\
BSD 2-Clause                                    & $19$  & CC0 1.0                                        & $6$  \\
GNU GPL v3.0                                    & $22$  & MPL 2.0                                        & $6$  \\
GNU GPL v2.0                                    & $3$   & GNU LGPL v2.1                                  & $4$  \\
ISC License                                     & $2$   & GNU LGPL v3.0                                  & $4$  \\
MIT-0                                           & $2$   & CC BY-NC 4.0                                   & $2$  \\
The Unlicense                                   & $3$   & BSD 3-Clause Clear                             & $1$  \\
Boost Software License 1.0                      & $1$   & Other / unidentifiable                         & $53$ \\
\midrule
\multicolumn{4}{l}{\textbf{Total: 990}} \\
\bottomrule
\end{tabular}
\end{table}
...
(Section~\ref{sec:method:cot}) is generated by Gemini-3-Preview using
...
\textbf{Stub: hyperparameters and token budgets to be finalized at
camera-ready time.} We list the categories of hyperparameters that this
appendix will cover:

\begin{itemize}\setlength\itemsep{0.1em}
  \item \emph{Mid-training schedule}: number of epochs over the FIM
        corpus, learning rate (peak and warm-up steps), weight decay,
        gradient-clip norm, optimizer, and batch size for each base
        model (Qwen2.5-Coder-7B/14B/32B, Qwen3-8B).
  \item \emph{Sequence packing}: maximum sequence length per base
        model (matching native pretraining context), sample-packing
        strategy, attention masking across packed boundaries.
  \item \emph{FIM tokenization}: native FIM sentinels per model
        family, rationale-vs.-body separator, position of the rationale
        inside the middle span.
  \item \emph{Post-training}: post-training pipeline configuration
        (R2E-Gym, SWE-Smith, SWE-Lego), number of post-training epochs
        (note the SWE-Lego $2$-epoch deviation discussed in
        Section~\ref{sec:exp:setup}), learning-rate schedule, and seed.
  \item \emph{Evaluation}: agent harness configuration per pipeline
        (R2E-Gym scaffold built on OpenHands; SWE-agent for SWE-Smith;
        OpenHands for SWE-Lego), maximum turn limits, decoding
        temperature, per-step timeout, and seed values.
\end{itemize}

% =============================================================================
\section{Full Qwen2.5-Coder Scaling Curves}
\label{app:scaling}

\textbf{Stub: full scaling-curve figure across 7B/14B/32B to be
finalized at camera-ready time.} The figure will plot SWE-Bench-Verified
and SWE-Bench-Lite pass rates vs.\ base-model parameter count for the
three configurations: instruction-tuned base, post-training only
(R2E-Gym reproduced), and FIM mid-training $+$ R2E-Gym; numerical values
are already given in Table~\ref{tab:main}.
