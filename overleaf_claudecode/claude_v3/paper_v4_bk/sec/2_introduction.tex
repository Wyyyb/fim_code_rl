...
A second set of results probes our motivating hypothesis. All benchmarks
in this paper are evaluated on checkpoints that have undergone the full
pipeline---either post-training alone (baseline) or mid-training followed
by post-training (ours)---because a mid-trained checkpoint without
subsequent instruction-style post-training has degraded
instruction-following and is not directly comparable to instruction-tuned
baselines. Under this protocol, agentic post-training alone substantially
regresses non-target capabilities at 14B, dropping LiveCodeBench, BFCL,
and $\tau$-bench by double-digit margins in some cases. Adding
mid-training before the same post-training restores
$+11.1$ points on LiveCodeBench, $+2.4$ on BFCL, and $+3.9$ on
$\tau$-bench. Because the mid-training corpus contains only Python code
with no tool-use data, mid-training has no a priori reason to affect
$\tau$-bench or BFCL; the fact that it does, in the same direction as the
in-domain SWE-Bench gains, provides direct evidence for the isomorphism
between function calls and agent tool calls.
...
\textbf{(3) Robustness evaluation.} We validate the recipe along three
axes---two model sizes within Qwen2.5-Coder-Instruct, two post-training
pipelines, and one alternative base (Qwen3-8B with SWE-Lego)---observing
consistent in-domain gains in every configuration.
