...
Usage
═══════════════════════════════════════════════════════════════════════
    python depfim_multi.py input.json output.json

Programmatic:
    from depfim_multi import process_samples_multi
    results = process_samples_multi(samples, verbose=True)

Requires depfim.py in the same directory (imports DependencyGraphBuilder,
FIMSelector, FunctionInfo, etc.).
"""
...
# Import base components from depfim.py
from depfim import (
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python depfim_multi.py input.json output.json")
        sys.exit(1)
    ...
    # ---- Stage 2: group-level post-processing ----
    group_entries = postprocess_multi_results(results)
    base, ext = os.path.splitext(output_path)
    group_output_path = f"{base}_groups{ext}"
    save_results(group_entries, group_output_path)

    # ---- Stage 3: distribution stats ----
    print_group_distribution_stats(group_entries)

    # python step_3_dep_select_multi_0227.py /data/yubo/datasets/process_data_output_0215/extracted_python_files_0215.json /data/yubo/datasets/process_data_output_0227/step_3_selected_multi_fim_functions_0227.json
